...
export default class DeleteBattleSessionCommand extends BaseCommand {
  constructor() {
    super(
      ...
      "Deletes the battle session for someone [DEV PERMS REQUIRED]",
      (data) => {
  ...
  override async invoke(interaction: CommandInteraction) {
    ...
    if (!ClientCache.battles.has(user!.id)) {
      return await interaction.editReply({
        content: "This user does not have a battle session at the moment.",
      });
    }
    ...
    await interaction.editReply({
      content: `Deleted battle session for ${user!.id}`,
    });
  }
}
