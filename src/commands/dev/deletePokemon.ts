...
export default class DeletePokemonCommand extends BaseCommand {
    constructor() {
        super('delete-pokemon', 'Deletes a pokemon from a user [DEV PERMS REQUIRED]', (data) => {
    ...
    override async invoke(interaction: CommandInteraction) {
        ...
        if (!trainer) return await interaction.editReply(`There is no trainer that has the id of ${user.id}`);

        const pokemon = Databases.PokemonCollection.find({ discordUserId: user.id });

        const pokemons = await pokemon.toArray();

        const pkmn = pokemons[id - 1];

        if (!pkmn) return await interaction.editReply(`There is no pokemon under that id.`);

        await Databases.PokemonCollection.deleteOne({ _id: pkmn._id });

        await interaction.editReply(`Deleted Level ${pkmn.level} ${pkmn.species} from ${user.username} (ID: ${user.id})`);
    }
}