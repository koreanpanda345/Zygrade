...
export default class MoveCommand extends BaseCommand {
    ...
    override async invoke(interaction: CommandInteraction) {
      ...
      if(!trainer) {
        await interaction.editReply({ content: 'Could not find any data for you!' });
        return;
      }

      const pokemons = await Databases.PokemonCollection.find({ discordUserId: interaction.user.id });

      const arr = await pokemons.toArray();

      if (!arr[id]) {
        await interaction.editReply({ content: `Doesn't seem like you have a pokemon under that id.`});
        return;
      }

      trainer.team[slot] = arr[id]._id as ObjectId;

      await Databases.TrainerCollection.updateOne({ discordUserId: interaction.user.id }, { $set: { team: trainer.team }});

      await interaction.editReply(`Added Level ${arr[id].level} ${arr[id].species} to your team in slot ${slot}`);
    }
}