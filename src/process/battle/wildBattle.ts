import { ActionRowBuilder, ButtonBuilder, ButtonStyle, Collection, Colors, CommandInteraction, ComponentType, EmbedBuilder } from "discord.js";
...
export default class WildBattleProcess extends BaseProcess {
    ...
    override async invoke(interaction: CommandInteraction) {
        ...
        const wildName = await ClientCache.invokeProcess('get-random-pokemon', trainer!.route);
        const wildLevel = await ClientCache.invokeProcess('get-random-level', trainer!.route, wildName);
        ...
        const wildPokemon: PokemonSchema = {
            ...
            moves: await ClientCache.invokeProcess('get-learnable-moves', wildName, wildLevel) as string[],
            ability: Dex.species.get(wildName).abilities[`${Math.floor(Math.random()) as 0 | 1}`]!,
            nature: (await ClientCache.invokeProcess('get-random-nature')).name,
        };

        const wild = await ClientCache.invokeProcess('pack-team', [{ pokemon: wildPokemon, species: Dex.species.get(wildName)}]);
        ...
        for (let i = 0; i < trainerPokemon.length; i++) {
            ...
            const stats = await ClientCache.invokeProcess('handle-stats', Dex.species.get(trainerPokemon[i].species), trainerPokemon[i]);
            console.log(stats);
            battle.set(`p1:team:${i}:level`, trainerPokemon[i].level);
            battle.set(`p1:team:${i}:species`, Dex.species.get(trainerPokemon[i].species).name);
        ...
        const wildStats = await ClientCache.invokeProcess('handle-stats', Dex.species.get(wildPokemon.species), wildPokemon);
        ...
        embed.setThumbnail(`https://play.pokemonshowdown.com/sprites/xyani/${wildName.toLowerCase()}.gif`);
        embed.setImage(`https://play.pokemonshowdown.com/sprites/xyani/${trainerPokemon[0].species.toLowerCase()}.gif`);
        embed.setFooter({ text: "Select a move to battle!" });

        const [p1HpBar, p1HpPercent] = filledBar(battle.get(`p1:team:0:stats:maxhp`) as number, battle.get(`p1:team:0:stats:hp`) as number, 20);
        const [p2HpBar, p2HpPercent] = filledBar(battle.get(`p2:team:0:stats:maxhp`) as number, battle.get(`p2:team:0:stats:hp`) as number, 20);
        embed.addFields(
            { name: `Level ${trainerPokemon[0].level} ${Dex.species.get(trainerPokemon[0].species).name}`, value: `HP: ${p1HpBar} (${Math.floor(Math.round(Number(p1HpPercent)))}%) [${battle.get(`p1:team:0:stats:hp`)}/${battle.get(`p2:team:0:stats:maxhp`)}]`, inline: true },
            { name: '\u200b', value: `\u200b`, inline: true },
            { name: `Level ${wildLevel} ${Dex.species.get(wildName).name}`, value: `HP: ${p2HpBar} (${Math.floor(Math.round(Number(p2HpPercent)))}%)`, inline: true },
        );
        ...
        for (const pokemonMove of trainerPokemon[0].moves) {
            ...
            button.setLabel(move.exists ? `${move.name} [PP: ${battle.get(`p1:team:0:moves:${move.id}:pp`)}/${battle.get(`p1:team:0:moves:${move.id}:maxpp`)}]` : '---');
        ...
        for (let i = 0; i < trainerPokemon.length; i++) {
            ...
            const button = new ButtonBuilder();
            button.setCustomId(`switch-${i + 1}`);
            button.setLabel(`${dex.name}`);
            button.setStyle(ButtonStyle.Secondary);
            button.setDisabled(battle.get(`p1:team:${i}:fainted`) || i == battle.get(`p1:current`));
        ...
        await interaction.editReply({ embeds: [embed], components: [moveRow, switchRow, optionsRow] });
            ...
            for (const line of chunk.split("\n")) {
                ...
                if (sections[1] === "win") {
                    ...
                    if (sections[2] !== interaction.user.username) {
                        ...
                        winEmbed.setTitle(`You failed to beat Level ${battle.get(`p2:team:0:level`)} ${battle.get(`p2:team:0:species`)}`);
                        winEmbed.setImage(`https://play.pokemonshowdown.com/sprites/xyani/${battle.get(`p2:team:0:species`).toLowerCase()}.gif`);
                        winEmbed.setColor("Red");

                        await interaction.editReply({ content: "Battle Done", embeds: [winEmbed], components: []});

                        ClientCache.battles.delete(interaction.user.id);
                        return;
                    }

                    winEmbed.setTitle(`You beat Level ${battle.get(`p2:team:0:level`)} ${battle.get(`p2:team:0:species`)}!`);
                    winEmbed.setImage(`https://play.pokemonshowdown.com/sprites/xyani/${battle.get(`p2:team:0:species`).toLowerCase()}.gif`);
                    winEmbed.setColor("Green");
                    
                    const catchButton = new ButtonBuilder().setCustomId("catch").setLabel("Catch Pokemon").setStyle(ButtonStyle.Success);
                    const leaveButton = new ButtonBuilder().setCustomId("leave").setLabel("Leave Pokemon").setStyle(ButtonStyle.Danger);

                    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(catchButton, leaveButton);

                    const wildData = await new PokemonClient().getPokemonByName(wildPokemon.species);
                    ...
                    const gainedExp = await ClientCache.invokeProcess('handle-gain-exp', wildData.base_experience, egg, favor, level, levelPlayer, ppower, share, originalTrainer, isPastEvolutionAmount);

                    await ClientCache.invokeProcess('handle-levels-and-exp', battle.get(`p1:team:0`), gainedExp);
                    const msg = await interaction.editReply({ content: "Battle Done", embeds: [winEmbed], components: [row]});
                    ...
                    mc.on("collect", async (i) => {
                        ...
                        if (i.customId === "catch") {
                            const neededExp = await ClientCache.invokeProcess("handle-growth-rate",
                                ((await new PokemonClient().getPokemonSpeciesByName(wildPokemon.species)).growth_rate.name),
                                wildPokemon.level
                            );
                            ...
                            const pokemon: PokemonSchema = {
                                ...
                                neededExp,
                                ivs: wildPokemon.ivs,
                                evs: wildPokemon.evs || { hp: 0, atk: 0, def: 0, spa: 0, spd: 0, spe: 0},
                            ...
                            embed.setThumbnail(`https://play.pokemonshowdown.com/sprites/xyani/${wildPokemon.species.toLowerCase()}.gif`);
                ...
                if (sections[1] === "turn") {
                    ...
                    embed.setImage(`https://play.pokemonshowdown.com/sprites/xyani/${battle.get(`p1:team:${battle.get(`p1:current`)}:species`).toLowerCase()}.gif`);
                    moveButtons = [];
                    for (const pokemonMove of trainerPokemon[battle.get('p1:current')].moves) {
                        console.log(pokemonMove);
                        const move = Dex.moves.get(pokemonMove);
                        if (!move.exists) continue;
                        const button = new ButtonBuilder();
                        button.setCustomId(`move-${move.id}`);
                        button.setLabel(move.exists ? `${move.name} [PP: ${battle.get(`p1:team:${battle.get(`p1:current`)}:moves:${move.id}:pp`)}/${battle.get(`p1:team:${battle.get(`p1:current`)}:moves:${move.id}:maxpp`)}]` : '---');
                        button.setDisabled(!move.exists);
                        button.setStyle(ButtonStyle.Primary);
                        moveButtons.push(button);
                    }
                    ...
                    for (let i = 0; i < trainerPokemon.length; i++) {
                        ...
                        const button = new ButtonBuilder();
                        button.setCustomId(i === current ? `switch-0` : `switch-${i + 1}`);

                        button.setLabel(`${dex.name}`);
                        button.setStyle(battle.get(`p1:team:${i}:fainted`) ? ButtonStyle.Danger : ButtonStyle.Secondary);
                        button.setDisabled(battle.get(`p1:team:${i}:fainted`) || i == current);
                ...
                const turn = battle.get('turn');
                if (battle.get(`${p1Path}:turn:${turn}:used`) && battle.get(`${p2Path}:turn:${turn}:used`))
                    embed.setDescription(`${battle.get(`${p1Path}:species`)} used ${battle.get(`${p1Path}:turn:${turn}:used`)}\n${battle.get(`${p2Path}:species`)} used ${battle.get(`${p2Path}:turn:${turn}:used`)}`);

                if (Number.isNaN(battle.get(`${p1Path}:stats:hp`))) continue;
                if (Number.isNaN(battle.get(`${p2Path}:stats:hp`))) continue;

                const [p1HpBar, p1HpPercent] = filledBar(battle.get(`${p1Path}:stats:maxhp`) as number, battle.get(`${p1Path}:stats:hp`) as number, 20);
                const [p2HpBar, p2HpPercent] = filledBar(battle.get(`${p2Path}:stats:maxhp`) as number, battle.get(`${p2Path}:stats:hp`) as number, 20);
                const p1StatsBoost = () => {
                    const str: string[] = [];
                ...
                embed.setFields(
                    { name: `Level ${battle.get(`${p1Path}:level`)} ${battle.get(`${p1Path}:species`)}`, value: `HP: ${p1HpBar} (${Math.floor(Math.round(Number(p1HpPercent)))}%) [${battle.get(`${p1Path}:stats:hp`)}/${battle.get(`${p1Path}:stats:maxhp`)}]`, inline: true},
                    { name: '\u200b', value: '\u200b', inline: true },
                    { name: `Level ${battle.get(`${p2Path}:level`)} ${battle.get(`${p2Path}:species`)}`, value: `HP: ${p2HpBar} (${Math.floor(Math.round(Number(p2HpPercent)))}%)`, inline: true},
                    { name: `Stat Boost`, value: `${p1StatsBoost()}`, inline: true },
                    { name: `\u200b`, value: '\u200b', inline: true },
                    { name: `Stat Boost`, value: `${p2StatsBoost()}`, inline: true },
                );
