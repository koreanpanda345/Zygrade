...
export default class KantoQuest5 extends BaseQuest {
  constructor() {
    super(
      "Beat 3 Bug Catchers in Viridian Forest",
      "kanto_quest_5",
      "beat",
      "You must defeat 3 Bug Catchers in Viridian Forest",
    );
    this.progress = [0, 3];
  }
  ...
  override async getRewards(userid: string) {
    ...
    const eeveeReward = {
      ...
      shiny: false,
      exp: 0,
      neededExp: await ClientCache.invokeProcess(
        "handle-growth-rate",
        (await new PokemonClient().getPokemonSpeciesByName("eevee")).growth_rate
          .name,
        5,
      ),
    ...
    await Databases.TrainerCollection.updateOne({ discordUserId: userid }, {
      $set: { money: trainer.money, allowedRoutes: trainer.allowedRoutes },
    });
